...
NHOSTS=$(yq -e '.NHOSTS' config.yaml)
num_node="${NHOSTS}"
num_gpus=$((${num_gpus_pernode} * ${num_node}))
## Data parallel size.
sp_size=$(yq -e '.sp_size' config.yaml)
## Data parallel size.
...
micro_batch_size=$(yq -e '.micro_batch_size' config.yaml)
# batch_size=2
###############################################################################
### Misc configs
log_interval=10
...
activation_checkpoint=$(yq -r '.activation_checkpoint' config.yaml)
...
num_workers=$(yq -e '.num_workers' config.yaml)
...
do
  ...
  #ssh $node 'source $EXP_HOME/miniconda3/etc/profile.d/conda.sh && conda activate .venv_train'
  gpu_count=$(ssh ${node} "nvidia-smi --query-gpu=name --format=csv,noheader | wc -l")
  echo "${node} slots=${gpu_count}"
done > "${hostfile}"
